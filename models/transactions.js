...
const createTransaction = (pk_transaction, fk_user, description, amount) => {
  try {
    let transaction = postgresql.public.one(
      `insert into transactions values ('${pk_transaction}', '${fk_user}', '${description}', '${amount}') returning *;`
    );
    return transaction;
  } catch (e) {
...
const updateTransaction = (pk_transaction, fk_user, description, amount) => {
  try {
    let user = postgresql.public.one(
      `update transactions set fk_user = '${fk_user}', description = '${description}', amount = '${amount}' where pk_transaction = '${pk_transaction}' returning *;`
    );
    return user;
  } catch (e) {
...
const getTransactionsPaginated = (page) => {
  try {
    let transaction = postgresql.public.many(
      `select * from transactions order by pk_transaction limit 5 offset ${
        (page - 1) * 5
      }`
    );
    return transaction;
